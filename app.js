...
const listings=[
 {id:'demo-masaki',title:'Sunlit 2-Bedroom Apartment in Masaki',location:'Masaki, Dar es Salaam',city:'Dar es Salaam',neighborhood:'Masaki',currency:'TZS',nightlyPrice:280000,bedrooms:2,bathrooms:2,maxGuests:4,propertyType:'apartment',rentalMode:'short_term',rating:4.9,reviewCount:28,coverUrl:'/assets/ezgif-frame-032.jpg',agentFirstName:'Amina',agentLastName:'Hassan',verifiedAgent:true,description:'A bright, calm apartment close to the waterfront, cafés, and the best of Masaki.',amenities:'Wi-Fi,Air conditioning,Parking,Kitchen',tag:'Guest favourite'},
 {id:'demo-mikocheni',title:'The Palm House in Mikocheni',location:'Mikocheni, Dar es Salaam',city:'Dar es Salaam',neighborhood:'Mikocheni',currency:'TZS',monthlyPrice:3200000,bedrooms:3,bathrooms:2,maxGuests:6,propertyType:'house',rentalMode:'long_term',rating:4.8,reviewCount:19,coverUrl:'/assets/ezgif-frame-084.jpg',agentFirstName:'James',agentLastName:'Mwangi',verifiedAgent:true,description:'A spacious family home with leafy outdoor space and easy access to schools and shops.',amenities:'Garden,Security,Parking,Laundry',tag:'Long-term favourite'},
 {id:'demo-oyster',title:'Quiet Garden Studio in Oyster Bay',location:'Oyster Bay, Dar es Salaam',city:'Dar es Salaam',neighborhood:'Oyster Bay',currency:'TZS',nightlyPrice:185000,bedrooms:1,bathrooms:1,maxGuests:2,propertyType:'studio',rentalMode:'short_term',rating:4.9,reviewCount:41,coverUrl:'/assets/ezgif-frame-156.jpg',agentFirstName:'Grace',agentLastName:'Kimaro',verifiedAgent:true,description:'A private garden studio for quiet city breaks, with natural light and a relaxed coastal feel.',amenities:'Wi-Fi,Air conditioning,Garden,Workspace',tag:'Most loved'},
 {id:'demo-arusha',title:'Modern Villa with Pool in Arusha',location:'Kijenge, Arusha',city:'Arusha',neighborhood:'Kijenge',currency:'TZS',monthlyPrice:4800000,bedrooms:4,bathrooms:3,maxGuests:8,propertyType:'villa',rentalMode:'long_term',rating:4.8,reviewCount:16,coverUrl:'/assets/ezgif-frame-220.jpg',agentFirstName:'Neema',agentLastName:'Mollel',verifiedAgent:true,description:'A generous modern villa with a private pool, mountain air, and room for the whole family.',amenities:'Pool,Garden,Fireplace,Parking',tag:'New on StayNest'},
 {id:'demo-kinondoni',title:'Warm 1-Bedroom Home in Kinondoni',location:'Kinondoni, Dar es Salaam',city:'Dar es Salaam',neighborhood:'Kinondoni',currency:'TZS',nightlyPrice:145000,bedrooms:1,bathrooms:1,maxGuests:2,propertyType:'apartment',rentalMode:'short_term',rating:4.7,reviewCount:12,coverUrl:'/assets/ezgif-frame-071.jpg',agentFirstName:'Daniel',agentLastName:'Mashauri',verifiedAgent:true,description:'A comfortable, well-connected apartment for work trips and easy city living.',amenities:'Wi-Fi,Workspace,Security,Kitchen',tag:''},
 {id:'demo-mwanza',title:'Lakeview Family Home in Mwanza',location:'Capri Point, Mwanza',city:'Mwanza',neighborhood:'Capri Point',currency:'TZS',monthlyPrice:2600000,bedrooms:3,bathrooms:2,maxGuests:6,propertyType:'house',rentalMode:'long_term',rating:4.6,reviewCount:9,coverUrl:'/assets/ezgif-frame-126.jpg',agentFirstName:'Baraka',agentLastName:'Said',verifiedAgent:true,description:'A welcoming home with broad lake views, generous bedrooms, and a peaceful neighborhood.',amenities:'Lake view,Parking,Garden,Security',tag:''},
 {id:'demo-dodoma',title:'Contemporary Retreat in Dodoma',location:'Area C, Dodoma',city:'Dodoma',neighborhood:'Area C',currency:'TZS',nightlyPrice:170000,bedrooms:2,bathrooms:2,maxGuests:4,propertyType:'house',rentalMode:'short_term',rating:4.8,reviewCount:14,coverUrl:'/assets/ezgif-frame-144.jpg',agentFirstName:'Asha',agentLastName:'Mrema',verifiedAgent:true,description:'A polished two-bedroom retreat with generous living space in a quiet central neighborhood.',amenities:'Wi-Fi,Air conditioning,Parking,Kitchen',tag:'New on StayNest'},
 {id:'demo-paje',title:'Coastal Bungalow near Paje Beach',location:'Paje, Zanzibar',city:'Zanzibar',neighborhood:'Paje',currency:'TZS',nightlyPrice:360000,bedrooms:2,bathrooms:2,maxGuests:4,propertyType:'bungalow',rentalMode:'short_term',rating:4.9,reviewCount:33,coverUrl:'/assets/ezgif-frame-201.jpg',agentFirstName:'Zawadi',agentLastName:'Ali',verifiedAgent:true,description:'A breezy island bungalow designed for slow mornings, beach days, and memorable stays.',amenities:'Beach access,Wi-Fi,Garden,Outdoor shower',tag:'Guest favourite'},
 {id:'demo-mbezi',title:'Family Townhouse in Mbezi Beach',location:'Mbezi Beach, Dar es Salaam',city:'Dar es Salaam',neighborhood:'Mbezi Beach',currency:'TZS',monthlyPrice:3900000,bedrooms:3,bathrooms:3,maxGuests:6,propertyType:'townhouse',rentalMode:'long_term',rating:4.7,reviewCount:11,coverUrl:'/assets/ezgif-frame-018.jpg',agentFirstName:'Peter',agentLastName:'Kweka',verifiedAgent:true,description:'A secure townhouse with flexible living space near the coast and everyday conveniences.',amenities:'Security,Parking,Generator,Kitchen',tag:''},
 {id:'demo-njiro',title:'Private Garden Flat in Njiro',location:'Njiro, Arusha',city:'Arusha',neighborhood:'Njiro',currency:'TZS',nightlyPrice:210000,bedrooms:2,bathrooms:2,maxGuests:4,propertyType:'apartment',rentalMode:'short_term',rating:4.8,reviewCount:22,coverUrl:'/assets/ezgif-frame-001.jpg',agentFirstName:'Rehema',agentLastName:'John',verifiedAgent:true,description:'A peaceful garden flat with thoughtful details, ideal for a weekend in Arusha or a longer visit.',amenities:'Wi-Fi,Garden,Parking,Workspace',tag:''}
];
...
async function togglePostLike(id,node){if(!requireAccount('like posts'))return;try{const liked=node.dataset.liked==='true',csrf=(await apiJson('/api/auth/csrf')).token,result=await apiJson(`/api/listings/${id}/like`,{method:liked?'DELETE':'POST',headers:{'X-CSRF-Token':csrf}});node.dataset.liked=String(result.liked);node.classList.toggle('liked',result.liked);const counter=node.querySelector('[data-post-likes]');if(counter)counter.textContent=`${result.likes} likes`}catch(error){showToast(error.message)}}
...
function render(filter='all'){const data=filter==='popular'?currentListings.filter(x=>Number(x.rating||0)>0||Number(x.reviewCount||0)>0):filter==='new'?currentListings.slice().sort((a,b)=>String(b.updatedAt||'').localeCompare(String(a.updatedAt||''))):filter==='short'?currentListings.filter(x=>x.rentalMode==='short_term'||x.rentalMode==='both'):filter==='long'?currentListings.filter(x=>x.rentalMode==='long_term'||x.rentalMode==='both'):currentListings;if(!grid)return;if(!data.length){grid.innerHTML='<div class="feed-empty">No published homes match these filters yet. Try another location or clear the filters.</div>';return}grid.innerHTML=data.map(x=>{const price=x.nightlyPrice?`${x.currency} ${Number(x.nightlyPrice).toLocaleString()} / night`:x.monthlyPrice?`${x.currency} ${Number(x.monthlyPrice).toLocaleString()} / month`:`${x.currency} ${Number(x.yearlyPrice||0).toLocaleString()} / year`;const image=safeMediaUrl(x.coverUrl||'/assets/ezgif-frame-018.jpg');const rating=Number(x.rating||0);return `<article class="listing-card reveal-on-scroll is-visible" data-listing-id="${escapeHtml(x.id)}"><div class="listing-photo"><img class="listing-card-image" src="${escapeHtml(image)}" alt="${escapeHtml(x.title)}" loading="lazy" onerror="this.onerror=null;this.src='/assets/ezgif-frame-018.jpg'"><button class="heart" data-id="${escapeHtml(x.id)}" aria-label="Save ${escapeHtml(x.title)}">♡</button>${x.propertyType?`<span class="rating">${escapeHtml(x.propertyType)}</span>`:''}</div><div class="listing-info"><h3>${escapeHtml(x.title)}</h3><div class="listing-meta">${escapeHtml([x.neighborhood,x.city].filter(Boolean).join(', '))} · ${escapeHtml(x.propertyType||'Home')} · ${x.bedrooms||0} bed${Number(x.bedrooms)===1?'':'s'}</div><div class="listing-price"><strong>${escapeHtml(price)}</strong> <span class="listing-meta">${rating?` · ★ ${rating.toFixed(1)} (${x.reviewCount||0})`: ' · New listing'}</span></div></div></article>`}).join('');grid.querySelectorAll('.heart').forEach(btn=>btn.addEventListener('click',event=>{event.stopPropagation();if(!requireAccount('save homes'))return;btn.classList.toggle('saved');btn.textContent=btn.classList.contains('saved')?'♥':'♡';showToast(btn.classList.contains('saved')?'Saved to your wishlist':'Removed from wishlist')}));grid.querySelectorAll('.listing-card').forEach(card=>card.addEventListener('click',e=>{if(e.target.closest('.heart'))return;void openListingDetails({id:card.dataset.listingId})}))}
async function loadListings(filters={}){if(!grid)return;grid.innerHTML='<div class="feed-empty">Finding published homes…</div>';try{const params=new URLSearchParams(filters);const result=await apiJson(`/api/listings/discover?${params}`);const live=Array.isArray(result.listings)?result.listings:[];const liveImages=new Set(live.map(item=>item.coverUrl).filter(Boolean));const trustedLive=live.length>=8&&liveImages.size===live.length&&live.every(item=>item.currency==='TZS'&&!/kado|burnette|best in town|^estate$/i.test(String(item.title||'')));currentListings=trustedLive?live:listings;render(document.querySelector('.chip.active')?.dataset.filter||'all')}catch(error){currentListings=listings;render(document.querySelector('.chip.active')?.dataset.filter||'all')}}
let toastTimer;
...
async function openListingDetails(x){try{const localIndex=listings.findIndex(item=>String(item.id)===String(x.id));const localListing=localIndex>=0?listings[localIndex]:null;const result=localListing?{listing:{...localListing,agentId:0,firstName:localListing.agentFirstName,lastName:localListing.agentLastName,saved:false},media:[{mediaType:'image',mediaUrl:localListing.coverUrl},{mediaType:'image',mediaUrl:`/assets/ezgif-frame-${String((localIndex*17+9)%240+1).padStart(3,'0')}.jpg`}],reviews:[{rating:localListing.rating,comment:'A comfortable, well-presented StayNest home with a thoughtful host.',firstName:'StayNest',lastName:'Guest'}],similar:[]}:await apiJson(`/api/listings/${encodeURIComponent(x.id)}/details`);const listing=result.listing;const media=Array.isArray(result.media)&&result.media.length?result.media:[{mediaType:'image',mediaUrl:listing.coverUrl}];const recent=JSON.parse(localStorage.getItem('staynest.recentListings')||'[]').filter(id=>Number(id)!==Number(listing.id));localStorage.setItem('staynest.recentListings',JSON.stringify([Number(listing.id),...recent].slice(0,12)));const mediaMarkup=media.map(item=>item.mediaType==='video'?`<video class="listing-detail-media" src="${escapeHtml(mediaUrl(item.mediaUrl))}" controls playsinline preload="metadata"></video>`:`<img class="listing-detail-media" src="${escapeHtml(mediaUrl(item.mediaUrl))}" alt="${escapeHtml(item.caption||listing.title)}" loading="lazy" onerror="this.onerror=null;this.src='${DEFAULT_AGENT_AVATAR}'">`).join('');const rating=Number(listing.rating||0);const amenities=String(listing.amenities||'').split(',').filter(Boolean);const reviewMarkup=result.reviews.length?result.reviews.map(review=>`<article class="review-item"><strong>${escapeHtml(`${review.firstName||''} ${review.lastName||''}`.trim()||'StayNest member')}</strong><span>★ ${Number(review.rating).toFixed(1)}</span><p>${escapeHtml(review.comment||'')}</p></article>`).join(''):'<p class="empty-copy">No reviews yet. Be the first guest to share your experience.</p>';openModal(`<p class="eyebrow">Listing details</p><h2>${escapeHtml(listing.title)}</h2><p>${escapeHtml(listing.city||'')} ${listing.neighborhood?`· ${escapeHtml(listing.neighborhood)}`:''}${rating?` · ★ ${rating.toFixed(1)} (${listing.reviewCount||0} reviews)`: ' · New listing'}</p><div class="listing-detail-media-grid">${mediaMarkup}</div>${Number.isFinite(Number(listing.latitude))&&Number.isFinite(Number(listing.longitude))?`<h3>Estate location</h3><div id="tenantListingMap" class="tenant-listing-map"></div><p class="map-status">Approximate map location for this estate.</p>`:`<p class="empty-copy">Location map is not available for this listing yet.</p>`}<p>${escapeHtml(listing.description||'')}</p><div class="feature-list"><div><b>01</b><span><strong>${listing.currency} ${Number(listing.nightlyPrice||listing.monthlyPrice||listing.yearlyPrice||0).toLocaleString()} ${listing.nightlyPrice?'/ night':listing.monthlyPrice?'/ month':'/ year'}</strong><small>${escapeHtml(listing.rentalMode||'Flexible stay')} · ${escapeHtml(listing.propertyType||'Home')} · ${listing.bedrooms||0} bedrooms · ${listing.bathrooms||0} bathrooms · Up to ${listing.maxGuests||'—'} guests</small></span></div></div>${amenities.length?`<h3>Amenities</h3><p>${amenities.map(item=>`<span class="amenity-pill">${escapeHtml(item)}</span>`).join(' ')}</p>`:''}<h3>Host</h3><p>${escapeHtml(`${listing.firstName||''} ${listing.lastName||''}`.trim()||listing.agencyName||'StayNest host')}${listing.verifiedAgent?' · Verified agent':''}</p><div class="listing-actions"><button class="primary" id="detailBook">Request booking <span>→</span></button><button class="outline-btn" id="detailSave">${listing.saved?'Saved':'Save favorite'}</button><button class="outline-btn" id="detailMessage">Contact agent</button><button class="outline-btn" id="detailCompare">Compare</button><button class="outline-btn" id="detailShare">Share</button><button class="outline-btn" id="detailReport">Report listing</button></div><h3>Reviews</h3>${reviewMarkup}<h3>Similar listings</h3>${result.similar.length?result.similar.map(item=>`<button class="search-result" data-similar-id="${item.id}"><img src="${safeMediaUrl(item.coverUrl||DEFAULT_AGENT_AVATAR)}" alt=""><span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.city)} · ${item.currency} ${item.nightlyPrice||item.monthlyPrice||'Contact agent'}</small></span></button>`).join(''):'<p class="empty-copy">No similar listings found yet.</p>'}`);const tenantMapHost=content.querySelector('#tenantListingMap');if(tenantMapHost){const latitude=Number(listing.latitude),longitude=Number(listing.longitude);try{const tenantMap=L.map(tenantMapHost,{scrollWheelZoom:false}).setView([latitude,longitude],16);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'&copy; OpenStreetMap contributors',maxZoom:19}).addTo(tenantMap);L.marker([latitude,longitude]).addTo(tenantMap);const resize=()=>tenantMap.invalidateSize({pan:false});requestAnimationFrame(()=>{resize();setTimeout(resize,250)})}catch(error){tenantMapHost.insertAdjacentHTML('afterend','<p class="form-error">The map could not be loaded. The listing location is still available.</p>');console.warn('Listing map unavailable:',error.message)}}content.querySelector('#detailBook').onclick=()=>{closeModal();openListing({...listing,id:listing.id,_booking:true,title:listing.title,location:listing.city,image:listing.coverUrl,rating:listing.rating,reviews:listing.reviewCount,tag:'Listing request'})};content.querySelector('#detailSave').onclick=async()=>{try{const csrf=(await apiJson('/api/auth/csrf')).token;const saved=content.querySelector('#detailSave').textContent==='Saved';if(saved){await apiJson(`/api/listings/${listing.id}/save`,{method:'DELETE',headers:{'X-CSRF-Token':csrf}})}else await apiJson(`/api/listings/${listing.id}/save`,{method:'POST',headers:{'X-CSRF-Token':csrf}});content.querySelector('#detailSave').textContent=saved?'Save favorite':'Saved';showToast(saved?'Removed from favorites':'Saved to favorites')}catch(error){showToast(error.message)}};content.querySelector('#detailMessage').onclick=()=>openAgentMessage(listing.agentId,`${listing.firstName||'Listing'} ${listing.lastName||'agent'}`);content.querySelector('#detailCompare').onclick=()=>{const selected=JSON.parse(localStorage.getItem('staynest.compareListings')||'[]').filter(item=>Number(item.id)!==Number(listing.id));selected.push({id:listing.id,title:listing.title,price:listing.nightlyPrice||listing.monthlyPrice||listing.yearlyPrice||'Contact agent',city:listing.city,bedrooms:listing.bedrooms,bathrooms:listing.bathrooms,rating:listing.rating});localStorage.setItem('staynest.compareListings',JSON.stringify(selected.slice(-3)));openModal(`<p class="eyebrow">Listing comparison</p><h2>Compare saved listings</h2><div class="compare-grid">${selected.slice(-3).map(item=>`<div><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.city||'')}<br>${item.price}<br>${item.bedrooms||0} bedrooms · ${item.bathrooms||0} bathrooms<br>★ ${Number(item.rating||0).toFixed(1)}</small></div>`).join('')}</div>`)};content.querySelector('#detailShare').onclick=async()=>{const url=`${location.origin}/index.html?listing=${listing.id}`;if(navigator.share)await navigator.share({title:listing.title,url});else await navigator.clipboard.writeText(url);showToast('Listing link copied')};content.querySelector('#detailReport').onclick=()=>openListingReport(listing.id);content.querySelectorAll('[data-similar-id]').forEach(button=>button.onclick=()=>openListingDetails({id:button.dataset.similarId}));}catch(error){showToast(error.message)}}
...
async function loadAgentShowcase() {
  ...
  try {
    ...
    const presentationPosts = (Array.isArray(result.posts) ? result.posts : []).filter(post => !/kado|burnette|phone store|best in town|estate$/i.test(`${post.firstName || ''} ${post.lastName || ''} ${post.title || ''}`));
    if (!presentationPosts.length) return;
    track.innerHTML = presentationPosts
      .map((post, index) => {
...
initStayNestConfig();
document.addEventListener('click',event=>{if(event.target.closest('#detailSave,#detailReport')&&!requireAccount('save or report listings')){event.preventDefault();event.stopImmediatePropagation();return}},true);
initCookieConsent();
